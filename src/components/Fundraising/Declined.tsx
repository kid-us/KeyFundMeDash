...
const Declined = () => {
  ...
  const { data: campaign, isLoading } = useQuery({
    ...
    queryFn: () =>
      axios
        .get<AllCampaigns>(`${baseUrl}fundraisers?status=declined`, {
          headers: {
      ...
      <div className="mt-10">
        ...
        <div>
          {campaign && campaign.length > 0 ? (
            <Campaign campaigns={campaign} status="declined" />
          ) : (
            <div className=" bg-white shadow p-6 rounded-xl">
              <p className="text-center">
                There are no active declined campaigns at the moment. When
